...
sendCustomerRefund Job {id, jobInfo} = withLogTag ("JobId-" <> id.getId) do
  ...
  riderConfig <- CQRC.findByMerchantOperatingCityId merchantOpCityId >>= fromMaybeM (InternalError $ "RiderConfig not found for mocId: " <> show merchantOpCityId.getId)
  let rescheduleTimeDiff = payoutConfig <&> (.timeDiff)
  eligibleBookingsList <- QFTB.findAllByCashbackSettled riderConfig.payoutBatchSize (Just 0) (Just False) (Just statusForRetry)
      ...
      case booking.payerVpa of
        Just payoutVpa -> do
          Redis.withWaitOnLockRedisWithExpiry (DAP.payoutProcessingLockKey booking.id.getId) 3 3 $ do
            ...
            if amount <= config.thresholdPayoutAmountPerOrder
              then do
                ...
                Redis.withWaitOnLockRedisWithExpiry (DAP.payoutProcessingLockKey booking.id.getId) 3 3 $ do
                  QFTB.updatePayoutStatusById (Just NEED_MANUAL_VERIFICATION) booking.id
            pure ()
        Nothing -> pure ()
    Nothing -> pure ()
    ...
    Left _ -> do
      ...
      if eligibleForRetryInNextBatch
        then Redis.withWaitOnLockRedisWithExpiry (DAP.payoutProcessingLockKey bookingId.getId) 3 3 $ do
          ...
          let status = if statusForRetry == NEED_MANUAL_VERIFICATION then PROCESSING else NEED_MANUAL_VERIFICATION
